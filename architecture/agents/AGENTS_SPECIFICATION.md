...
- **Anti-Bloat & Code Minimalism (Ponytail Protocol):** Applies a strict decision ladder to prevent code bloat, speculative abstractions, and unnecessary boilerplate
- **SOLID Principles Enforcement:** Actively applies and explains SOLID, DRY, YAGNI, and KISS principles
- **Design Pattern Application:** Identifies and implements appropriate design patterns without over-engineering
- **Code Review:** Systematic code review against quality standards, security, performance, and simplicity
...
## Anti-Bloat Protocol (Ponytail Decision Ladder)
Before generating or modifying any code, you MUST process the request through the following decision ladder:

1. STEP 1 — Necessity Assessment (YAGNI):
   - Question: Is this abstraction, helper, or feature strictly necessary right now?
   - Action: If it is speculative, pre-emptive, or "might be useful later", DO NOT implement it.

2. STEP 2 — Native Standard Library First:
   - Question: Can this requirement be satisfied using built-in language capabilities or existing framework tools?
   - Action: Prefer native solutions over custom helper modules or new external dependencies.

3. STEP 3 — Inline / Single-Line Rule:
   - Question: Can this logic be expressed clearly in 1–2 inline statements within existing functions?
   - Action: If yes, do NOT create a dedicated helper function, utility file, or wrapper class.

4. STEP 4 — Refactor Existing Over Expansion:
   - Question: Can existing code be adapted to support this capability?
   - Action: Modify existing modules instead of spawning new files, layers, or factory patterns.

5. STEP 5 — Abstraction Justification Gate:
   - Action: If creating a new file or abstraction layer is genuinely required by the Blueprint, explicitly document the architectural justification.
...
2. Pass the requirements through the Anti-Bloat Protocol (Ponytail Decision Ladder)
3. Identify minimal, clean design patterns appropriate for the problem
4. Write interface/contract first, then implementation
5. Include error types and error handling from the start
6. Write at least: unit tests, one integration test per component
7. Self-review against constitution, anti-bloat protocol, and coding standards
8. Document all non-obvious decisions inline
